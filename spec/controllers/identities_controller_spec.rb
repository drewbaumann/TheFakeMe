...
describe IdentitiesController do
  ...
  def mock_identity(stubs={})
    (@mock_identity ||= mock_model(Identity).as_null_object).tap do |identity|
      identity.stub(stubs) unless stubs.empty?
    end
  end

  describe "GET index" do
    it "assigns all identities as @identities" do
      Identity.stub(:all) { [mock_identity] }
      get :index
      assigns(:identities).should eq([mock_identity])
    end
  end

  describe "GET show" do
    it "assigns the requested identity as @identity" do
      Identity.stub(:find).with("37") { mock_identity }
      get :show, :id => "37"
      assigns(:identity).should be(mock_identity)
    end
  end

  describe "GET new" do
    it "assigns a new identity as @identity" do
      Identity.stub(:new) { mock_identity }
      get :new
      assigns(:identity).should be(mock_identity)
    end
  end
  ...
  describe "POST create" do
    ...
    describe "with valid params" do
      it "assigns a newly created identity as @identity" do
        Identity.stub(:new).with({'these' => 'params'}) { mock_identity(:save => true) }
        post :create, :identity => {'these' => 'params'}
        assigns(:identity).should be(mock_identity)
      end

      it "redirects to the created identity" do
        Identity.stub(:new) { mock_identity(:save => true) }
    ...
    describe "with invalid params" do
      it "assigns a newly created but unsaved identity as @identity" do
        Identity.stub(:new).with({'these' => 'params'}) { mock_identity(:save => false) }
        post :create, :identity => {'these' => 'params'}
        assigns(:identity).should be(mock_identity)
      end

      it "re-renders the 'new' template" do
        Identity.stub(:new) { mock_identity(:save => false) }
        post :create, :identity => {}
  ...
  describe "PUT update" do
    ...
    describe "with valid params" do
      it "updates the requested identity" do
        Identity.should_receive(:find).with("37") { mock_identity }
        mock_identity.should_receive(:update_attributes).with({'these' => 'params'})
        put :update, :id => "37", :identity => {'these' => 'params'}
      end

      it "assigns the requested identity as @identity" do
        Identity.stub(:find) { mock_identity(:update_attributes => true) }
        put :update, :id => "1"
        assigns(:identity).should be(mock_identity)
      end
    ...
    describe "with invalid params" do
      it "assigns the identity as @identity" do
        Identity.stub(:find) { mock_identity(:update_attributes => false) }
        put :update, :id => "1"
        assigns(:identity).should be(mock_identity)
      end
  ...
  describe "DELETE destroy" do
    ...
    it "redirects to the identities list" do
      Identity.stub(:find) { mock_identity }
      delete :destroy, :id => "1"
